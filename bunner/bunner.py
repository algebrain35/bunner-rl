...
class Bunner(MyActor):
    ...
    def update(self, action=None):
        ...
        if action is None:
            ...
            # If a control input is provided by an agent, add it to the input queue
...
class Game:
    ...
    def get_hedge_penalty(self):
        ...
        for i, row in enumerate(self.rows):
            if row.y == y:
                idx = i
                break
        if idx != -1 and idx + 1 < len(self.rows):
            for child in self.rows[idx+1].children:
                if not isinstance(child, Hedge):
                    continue
                if child.x >= x - (W / 2) - 4 and child.x < x + (W / 2) + 4:
                    return -0.1
    ...
    def get_log_reward(self):
        ...
        for row in self.rows:
            if row.y != y:
                ...
                if isinstance(row, Water):
                    for log in row.children:
                        if log == row.collide(self.bunner.x, -4):
                            return 1.0
        return 0.0
    ...
    def game_step(self, action):
            import cv2
            import numpy as np
            y_prev = self.bunner.min_y
            self.update(action)
            self.draw()
            
            y_curr = self.bunner.min_y
            hedge_penalty = self.get_hedge_penalty()
            is_furthest = 1.0 if y_curr < y_prev else 0.0

            while self.bunner.timer > 0 and self.bunner.state == PlayerState.ALIVE:
                self.update()
                self.draw()
            log_reward = self.get_log_reward() if is_furthest > 0 else 0.0
            print(f"Log reward: {log_reward}")
            next_state = pygame.surfarray.array3d(pygame.display.get_surface())
            reward = 0.0
            dist = self.bunner.y - self.scroll_pos + HEIGHT + 80 
            penalty = (HEIGHT / dist) * 0.1
            reward += log_reward + hedge_penalty + is_furthest
            terminal = self.bunner.state != PlayerState.ALIVE
            if terminal:
                reward = -5.0
                while not self.bunner.timer < 0:
                    self.update()
                    self.draw()
            else:
                reward += penalty
            ...
            print(f"Reward: {reward} Y Prev : {y_prev} Y Curr: {y_curr}")
            gray = cv2.cvtColor(next_state, cv2.COLOR_RGB2GRAY)
            resized = cv2.resize(gray, (80, 80), interpolation=cv2.INTER_AREA)
            normalized = resized.astype('float32') / 255.0 
            
            state_matrix = np.stack([normalized]*4, axis=-1)
            return state_matrix, reward, terminal
...
dqn = DQN(5, 50000, 1e-6, 0.9999, 0.02, 0.01, 150000, 20000)
#dqn.load_weights("./bunner/training/main.weights.h5")
class State(Enum):
    MENU = 1
    PLAY = 2
    GAME_OVER = 3
...
def update():
    global state, game, high_score, dqn_state
    
    if state == State.MENU:
        if key_just_pressed(keys.SPACE):
            state = State.PLAY
            import numpy as np
            dqn_state = np.zeros((80, 80, 4)) 
        ...
        if game.bunner.state != PlayerState.ALIVE and game.bunner.timer < 0:
            ...
            next_state = dqn.train_step(dqn_state, game.game_step)
            dqn_state = next_state
